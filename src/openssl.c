...
GLOBAL ULONG RangeSeed;
# else
ULONG RangeSeed = 0x234533;
# define RangeRand(r) rand()
# endif
#endif
...
init_prng (void)
{
  ...
  {
    /**
     * This is moslty what our OpenSSL for AmigaOS (http://amiga.sf.net/OpenSSL)
     * is doing already, but added here just in case WGet is compiled with
     * another distribution...
     */

    ULONG rnd,ThisTask = (ULONG) FindTask(NULL);
    BOOL ok = 0;

    BPTR fd;
    APTR WindowPtr = ((struct Process *)ThisTask)->pr_WindowPtr;
    ((struct Process *)ThisTask)->pr_WindowPtr = (APTR) -1;

    if((fd = Open( "RANDOM:r10", MODE_OLDFILE )))
    {
      if(Read( fd, namebuf, sizeof(namebuf)) > 0)
      {
	RAND_seed(namebuf,sizeof(namebuf));
	ok = 1;
      }
      Close(fd);
    }
    ((struct Process *)ThisTask)->pr_WindowPtr = WindowPtr;
    ...
    if( ok == 0 ) {
      ...
      logprintf (LOG_NOTQUIET, _("NOTE: Using build-in random seed...\n"));

      rnd = RangeRand(42928);
      RangeSeed ^= ThisTask << (rnd&0x1a);

	for( i = 0 ; i < sizeof(namebuf) ; i++ ) {
		if(!(i%2)) {
			DateStamp(&ds);
			rnd = (ULONG) ((RangeRand(63465) ^ ds.ds_Tick) | ((RangeSeed<<11) ^ (rnd>>7)));
			RangeSeed ^= rnd;
		}
		namebuf[i] = (unsigned char) rnd;
		rnd >>= 8;
	}
...
openssl_close (int fd, void *arg)
{
  ...
  closesocket (fd);
#elif defined(_AMIGASF) && defined(__libnix__)
  CloseSocket(fd);
#else
  close (fd);
#endif
