...
{
#ifdef _AMIGASF
  return xstrdup("SYS:Prefs/Env-Archive");
#else /* AmigaOS */
  ...
  if (!file)
    {
	  ...
	  if(NameFromLock(lock,path,sizeof(path)))
	    {
	      if(AddPart( path, "wgetcfg", sizeof(path)))
	        {
...
%s: %s: Invalid restriction %s,\n\
    use [unix|amiga|amiga-strict|windows],[lowercase|uppercase],[nocontrol],[ascii].\n"),
                   exec_name, com, quote (val));
          return false;
        }
