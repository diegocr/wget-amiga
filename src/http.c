...
#include <exec/execbase.h> /* SysBase */
#include <proto/exec.h> /* Supervisor */
#ifndef __amigaos4__
# include <proto/expansion.h> /* FindConfigDev */
#else
# define FindConfigDev(a,b,c) (0)
#endif
static unsigned char __aUserAgent[64],*__aUserAgentPtr = NULL;
#define SET_USER_AGENT(req) do {                                         \
  if (!opt.useragent) {                                                  \
    if(__aUserAgentPtr == NULL)                                          \
     snprintf(__aUserAgentPtr=__aUserAgent,sizeof(__aUserAgent),         \
                "Wget/%s (%s %ld.%ld; Amiga.sf Release)",                \
                        version_string,                                  \
			(FindResident("MorphOS") ? "MorphOS":            \
			((((struct Library *)SysBase)->lib_Version > 49) \
			? "AmigaOS4":((FindConfigDev(NULL, 1803, -1)     \
			|| FindConfigDev(NULL, 2011, 96))?"AmigaOS/UAE"  \
			:"AmigaOS"))),                                   \
			((struct Library *)SysBase)->lib_Version,        \
			((struct Library *)SysBase)->lib_Revision);      \
    request_set_header (req, "User-Agent",__aUserAgentPtr,rel_none);     \
  } else if (*opt.useragent)                                             \
    request_set_header (req, "User-Agent", opt.useragent, rel_none);     \
} while (0)
#else /* def __VMS */
...
#ifdef _AMIGASF
	setFComment( hstat.local_file, u->url);
#endif
