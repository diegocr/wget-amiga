...
static struct cmdline_option option_data[] =
  {
    ...
    { "dot-files", 0, OPT_BOOLEAN, "dotfiles", -1 },
    ...
    { "max-filename-length", 0, OPT_VALUE, "maxfilenamelength", -1 },
    { "max-redirect", 0, OPT_VALUE, "maxredirect", -1 },
    { "mirror", 'm', OPT_BOOLEAN, "mirror", -1 },
    { "no", 'n', OPT__NO, NULL, required_argument },
    { "no-clobber", 0, OPT_BOOLEAN, "noclobber", -1 },
#if _AMIGASF
    { "no-file-comments", 0, OPT_BOOLEAN, "nofilecomments", -1 },
#endif
...
Download:\n"),
  ...
  -Q,  --quota=NUMBER            set retrieval quota to NUMBER.\n"),
    ...
    N_("\
       --no-file-comments        do not set the url as comment on the file.\n"),
    N_("\
...
#if defined(SYSTEM_WGETRC) && !defined(_AMIGASF)
  printf (_("    %s (system)\n"), SYSTEM_WGETRC);
#endif
...
#if defined(_AMIGASF) && !defined(__amigaos4__) && 0
...
int openSockets( void )
{
  ...
  SocketBaseTags(
	...
	SBTM_SETVAL(SBTC_LOGTAGPTR),                 (ULONG) PACKAGE,
  TAG_DONE );
...
#ifdef fclose
#undef fclose
int fclose_debug(FILE *fd,const char *func,const int line)
{
  int rc;
  DEBUGP(("Closing fd %x - %s:%d\n",fd,func,line));
  rc = fclose(fd);
  DEBUGP(("FD %x closed\n",fd));
  return rc;
}
